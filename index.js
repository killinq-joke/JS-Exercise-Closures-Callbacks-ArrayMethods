...
function processSum(numberList, callback) {
  return callback(numberList.reduce((acc, numberList) => {
    return acc + numberList;
  },0)
  )
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter(currentItem => currentItem.shirt_size === tShirtSize);
...
function tallyUpDonations(runners) {
  return runners.reduce((acc, currentItem, index, array) => {
    return acc + currentItem.donation;
  }, 0);
}
